...
jwork.fileUpload.fileDialogCompleteHandler = function(numFilesSelected, numFilesQueued) {
	try {
		console.log("@@@@@@ fileDialogCompleteHandler @@@@@@"
				+ "\n역할 : 자동 업로드 기능");
...
jwork.fileUpload.uploadStartHandler = function(file) {
	...
	try {
		console.log("uploadStartHandler() > startUpload()");
		this.startUpload();
...
jwork.fileUpload.cancelUploadHandler = function(event, imgButton) {
	console.log("@@@@@@ cancelUploadHandler @@@@@@"
			+ "\n역할 : 파일 삭제 버튼 클릭시"
			+ "\nevent : " + event);
...
jwork.fileUpload.uploadProgressHandler = function(file, bytesLoaded, bytesTotal) {
	...
	try {
		...
		console.log("fileAreaTable : " + JSON.stringify(fileAreaTable));
		var progressContainer = fileAreaTable.find("#progressBar_"+file.id);
		console.log("progressContainer : " +  JSON.stringify(progressContainer));
		...
		console.log("progressBarInProgress : " +  JSON.stringify(progressBarInProgress));
...
jwork.fileUpload.uploadCompleteHandler = function(file) {
	...
	if (this.getStats().files_queued === 0) {
		...
		console.log("uploadCompleteHandler() > uploadFinished()");
	}
};
...
jwork.fileUpload.queueCompleteHandler = function() {
	console.log("@@@@@@ queueCompleteHandler @@@@@@"
			+ "\n역할 : 파일이 모두 전송되었을 경우");
};
...
jwork.fileUpload.prototype.getFileId = function() {
	console.log("@@@@ prototype.getFileId @@@@"
			+ fileId );
	return this.fileId;
};
...
jwork.fileUpload.prototype.setFileId = function(fileId) {
	console.log("@@@@ prototype.setFileId @@@@"
			+ fileId );
	this.fileId = fileId;
};
		...
		success : function(result) {
			...
			case "success":
				console.log("[getFileList success]"
						+ "\nresult : " + JSON.stringify(result.fileList));
				console.log("getFileList() > insertFileList(result.fileList)")
...
jwork.fileUpload.prototype.insertFileList = function(fileList) {
	console.log("@@@@@@ insertFileList @@@@@@"
			+ "\n역할 : 서버에서 받아온 파일 리스트의 정보를 현재 객체와 페이지에 저장화"
...
jwork.fileUpload.prototype.setOptions = function(options) {
	console.log("@@@@@@ setOptions @@@@@@@"
			+ "\n역할 : 사용자가 options에 명시한 내용으로 값 변경",
			+ "options : " + options);
...
jwork.fileUpload.prototype.getFileTypeForSwfUpload = function() {
	...
	console.log("@@@@@@ getFileTypeForSwfUpload @@@@@@" +
	"\n역할 : File 타입 값 반환 (SwfUpload 객체에서 요구하는 값)" +
	"\n" + "fileType : " + fileType);
	return fileType;
};
...
jwork.fileUpload.prototype.getFileTypeDescForSwfUpload = function() {
	console.log("@@@@@@ getFileTypeDescForSwfUpload @@@@@@@"
			+ "\n역할 : File 타입에 대한 설명 반환 (SwfUpload가 화면에 보여주는 파일 유형 값)");
	var fileTypeDesc;
	if(this.fileType == "all") {
...
jwork.fileUpload.prototype.initHtml = function() {
	...
	console.log("@@@@@@ initHtml @@@@@@@"
			+"\n역할 : 화면에 파일업로드 컴포넌트 표시");
	jQuery.ajax({
		url : this.htmlUrl, // htmlUrl : this.ctx(프로젝트명)+"/jfile/jfileuploadForm.do"
		async : false,
		success : function(resultText, status) {
			element.html(resultText); // jfileuploadForm.jsp
		},
		error : function(xhr, ajaxSettings, thrownError){
			if (xhr.status == 901) {
				alert("세션이 종료 되었습니다.");
...
jwork.fileUpload.prototype.appendFileRow = function(file, isSaved) {
	console.log("@@@@@@ appendFileRow @@@@@@@"
			+"\n역할 : 파일 추가시 컴포넌트 화면에 해당 파일 아이템 표시");
	var element = jQuery("#" + this.elementId);
...
jwork.fileUpload.prototype.changePreview = function(fileId, seq) {
	console.log("jwork.fileUpload.prototype.changePreview   >>> 파일 미리보기 이미지를 변경 ");
	if(!fileId) {
		fileId = "";
	}
	if(!seq) {
...
jwork.fileUpload.prototype.changePreviewByUrl = function(imageUrl) {
	console.log("jwork.fileUpload.prototype.changePreviewByUrl  >>> 파일 미리보기 이미지를 변경(경로 직접 입력) ");
	var element = jQuery("#" + this.elementId);
	var img = element.find("#spanThumbnail").find("img").get(0);
...
jwork.fileUpload.prototype.showPreview = function() {
	console.log("jwork.fileUpload.prototype.showPreview >>> 파일 미리보기 이미지를 보임 ");
	var element = jQuery("#" + this.elementId);
	var img = element.find("#spanThumbnail").find("img").get(0);
	jQuery(img).show();
};
...
jwork.fileUpload.prototype.hidePreview = function() {
	console.log("jwork.fileUpload.prototype.hidePreview >>> 파일 미리보기 이미지를 숨김 ");
	var element = jQuery("#" + this.elementId);
	var img = element.find("#spanThumbnail").find("img").get(0);
	jQuery(img).hide();
};
...
jwork.fileUpload.prototype.deleteFileItem = function(imageButton) {
	console.log("jwork.fileUpload.prototype.deleteFileItem >>> 삭제 버튼 클릭시 해당 파일을 삭제 처리 ");
...
jwork.fileUpload.prototype.beforeUpload = function() {
	console.log("@@@@@@ beforeUpload @@@@@@@" +
			"\n역할 : 파일 업로드 fileId 요청");
	var fileId; // 데이터 값을 받을 fileId를 선언 (beforeUpload 내에서 만)
	var fileUpload = this;
	...
	jQuery.ajax({
		...
		success: function(result){
			switch(result.JsonResultCode){
				case "success" :
					fileId = result.fileId; // 데이터를 서버에서 받아서 fileId에 저장
					console.log("[success : 데이터를 서버에서 받아서 fileId에 저장]"
							+ "\nresult : "
	...
	console.log("this.fileId(값 받기 전) : " + this.fileId);
	this.fileId = fileId; // 받아온 fileId를 전역 변수를 카리키는 fileId에 저장
	console.log("this.fileId(서버 값 받은 후) : " + this.fileId);
	if(this.isKeepSession == true) {
		this.checkUploading();
	}else{
		//this.checkUploadingOneTime();
	}
	console.log("beforeUpload() > startUpload()");
...
jwork.fileUpload.prototype.startUpload = function() {
	...
	if(this.swfu.getStats().files_queued === 0) {
		console.log("startUpload() > uploadFinished()")
		this.uploadFinished();
		...
		console.log("startUpload() > swfu.startUpload()")
...
jwork.fileUpload.prototype.uploadFinished = function() {
	...
	jQuery.ajax({
		...
		dataType: "json",
		success: function(result){
			console.log("[uploadFinished success]" +
						"\n rseult>>"
					+ JSON.stringify(result));
